...
RESULTS_FILE = "scripts/results_quantization.csv"
...
def test_on_data(data, name, results_list):
    ...
    offset = 100.0 # Assuming typical camera offset, or we can use min(data)
    
    print(f"Testing on {name} (Shape: {data.shape}, Type: {data.dtype})...")
...
def run_tests():
    ...
    if os.path.exists(REAL_DATA_PATH):
        try:
            ...
            codec = codecs.HuffmanQuant16(offset=100.0, scale=1.0)
            # Take middle frame
            if real_data.ndim == 3:
            ...
            decomp_slice = codec.decode(comp_slice)
            
            np.save("scripts/vis_original.npy", slice_data)
            np.save("scripts/vis_decompressed.npy", decomp_slice)
